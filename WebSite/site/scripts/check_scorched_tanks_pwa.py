...
def main() -> None:
    ...
    assert_condition(
        'id="install-button" class="button" type="button">' in html,
        "Install button must remain clickable before beforeinstallprompt fires.",
    )
    assert_condition(
    ...
    assert_condition(
        '"./original.js?v=bug043-install"' in service_worker,
        "Service worker cache must include the current original-runtime script.",
    )
