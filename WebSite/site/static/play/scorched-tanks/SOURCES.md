...
No proprietary Kickstart ROM is bundled. The browser-native artillery port is kept only as a labeled fallback and must not be reported by Workflow as the exact original game. Local fallback proof, 2026-05-01: a real browser canvas click fired a Big Shot, audio state was `running`, and the shot hit Kade for 59.7 damage (`output/scorched-compat-hit-tank-audio-local.png`).
