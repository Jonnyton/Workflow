...
- **Visual DNA:** dark canvas (ink `#1a1a2e`), ember accent (`#e94560`), violet sigil ring (`#533483`). One hero graphic: the summoning sigil in `assets/logo-mark.svg`. Fraunces display + IBM Plex Sans/Mono. No emoji. No gradients as fills. No icons without intentional substitution (we use Lucide via CDN — flagged).
...
- `assets/logo-mark.{svg,png}` — the brand mark.
...
4. Place the sigil mark as a bottom-right watermark at 0.06 opacity when a hero feels empty.
...
3. Use hairline borders (`--border-1`), never 2px. Use the sigil-glow (`--glow-sigil`) for hover / focus, not shadow.
...
**Making a slide deck:**
This brand has no existing slide template. If asked to make slides, ask the user whether they want a bespoke template built, flag that no prior deck exists, and build from the visual foundations (dark canvas, Fraunces display, the sigil as the one graphic per slide).
...
- Hover = sigil glow, not shadow. Press = 1px translate, not scale.
