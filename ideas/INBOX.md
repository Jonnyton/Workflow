...
- [2026-06-25] (source: user-chat, owner: unassigned, status: captured) BUG: BUG-029 chain-drain detection broken by FEAT-006 effective_chain narrowing. In router.call(), effective_chain removes unregistered providers BEFORE the drain check, so all_api_providers_in_cooldown(chain) sees only ollama-local and returns False -> CHAIN_DRAINED warning + empty-local backoff never fire. Repro: tests/test_provider_router_bug029.py::TestChainDrainBackoff (3) + test_bug029_chain_drain.py::test_chain_drain_emits_warning fail on clean origin/main f890d6ed. Likely stale-test vs prod (cooled-down providers are usually registered) but the drain check should use the pre-effective_chain chain. Found during loop-auth-router-quarantine Slice 2.
