...
5. **`node_lookup_fn` wiring (G.1 follow-up #1, R4).** In `fantasy_author/__main__.py:_run_graph` (or earlier at `DaemonController.start`), construct the lookup function and pass it to `NodeBidProducer`:
   ...
   - Exception isolation: lookup raises → WARN log + bid emitted (fail-open, R4).
...
5. **`node_lookup_fn` fail-open on exception (R4).** Lookup raises → producer logs + emits bid; executor-side sandbox catches. Test (new): patch `node_lookup_fn` to raise; submit bid; run producer; assert bid emitted + WARN log.
...
| `workflow/producers/node_bid.py` | EDIT — `node_lookup_fn` wired into startup path; fail-open on exception | ~20 lines |
...
- `node_lookup_fn` wired + fail-open (invariant 5). G.1 follow-up #1 resolved.
...
A. `fantasy_author/__main__.py:DaemonController.start` or `_run_graph`. Constructs function from disk walk OR SQLite registry (same path as the executor's existing lookup). Passes into `NodeBidProducer(node_lookup_fn=...)` at registration. Fail-open on exception (R4 invariant 5). Rationale: daemon-lifecycle code already owns producer registration (Phase F + G). Injecting the lookup there keeps the wiring in one place. SQLite-vs-disk choice inherits Phase G Concern ("node lookup uses disk walk, not SQLite registry") — keep disk walk for v1, add SQLite awareness later if needed.
...
- **R4 `node_lookup_fn` fail-open** — if lookup raises, producer should NOT block the cycle. Reviewer tests with a raising-lookup mock.
