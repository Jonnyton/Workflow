# Community Branches Phase 3 — Graph Runner

**Status:** proposed (awaiting lead approval)
**Depends on:** Phase 2 (done — MCP builder surface), #29 (no action exists to execute a registered node)
**Unblocks:** Phase 4 (eval + iteration), multi-domain validation of engine thesis

## Goal

Compile a validated `BranchDefinition` into a live LangGraph + SqliteSaver and run it against user-supplied inputs. Output must be observable step-by-step so Phase 4 can judge and iterate. Long runs must checkpoint and resume. Non-fantasy domains (research workflow, recipe tracker, etc.) must run end-to-end without any fantasy-specific node being required.

## Design decisions

1. **Compiler is a pure function.** `BranchDefinition → StateGraph` lives in `workflow/graph_compiler.py`. No side effects. Takes a validated branch + resolved tool allowlist, returns a compiled StateGraph. Compiler failures are programmer errors, not user errors (user errors are caught by `validate_branch` in Phase 2).
...
4. **Sandbox model mirrors existing `extensions` approval.**
   ...
   - Conditional edges run a predicate synthesized from the source node's `output_keys` — no user-code routing function in v1. Routing reads a single declared output key and maps its value through the `conditions` dict. Richer routers are Phase 4.

5. **Runs are async by default.** Phone-based users cannot hold a 30-minute chat open. `run_branch` returns immediately with a `run_id`; execution happens in a background task within the Universe Server process. Clients poll `get_run` or call `stream_run` for incremental updates.
...
**Ship Phase 3:** the 6 MCP actions above, the compiler, async runner with checkpointing, step event emission, cooperative cancel.

**Defer to Phase 4:** side-by-side run comparison, per-node quality scoring, re-run from checkpoint with edits, human-in-the-loop interrupts. Rerunning a whole branch from scratch after an edit is acceptable for v1.
...
- **#29 (no execution action)** — this spec IS that. Close #29 with Phase 3 merge.
- **#22 (commit write-back)** — done. Phase 3 does not touch commit pipeline.
...
- **Source-code sandbox escape risk.** The existing regex guard is weak (blocks `subprocess` in source text but not `__builtins__['sub'+'process']`). For Phase 3 merge, keep host-approval gate as primary defense and treat pattern check as belt-and-suspenders. A proper sandbox (resource limits, restricted builtins) is future work, tracked as a new concern.
...
1. Recipe-tracker branch from Phase 2 vignette runs end-to-end via `run_branch`, produces a final state readable via `get_run_output`.
