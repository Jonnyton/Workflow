...
6. `should_continue_universe` reads `health.stopped`. False → back to `foundation_priority_review` (next cycle). True → `END`. The `universe_cycle → foundation_priority_review` loop is not a validator-rejecting cycle — every node in the cycle reaches `END` through this conditional edge, which is why the fantasy graph (if re-expressed as a BranchDefinition) would pass `BranchDefinition.validate()`'s cycle-has-exit check at `workflow/branches.py:608-618`.
