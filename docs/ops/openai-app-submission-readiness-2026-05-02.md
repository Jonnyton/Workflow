...
- Test cases should pass in ChatGPT web and mobile.
...
3. `Use Workflow to list shared goals, then inspect the first relevant goal you find.`
4. `Use Workflow to list available universes, then inspect the active one.`
5. `Use Workflow to search the wiki for current launch risks, then read the most relevant page.`
6. `Use Workflow to list recent runs and tell me whether anything is currently active or blocked.`
7. `Use Workflow to propose a public goal named 'Onboard new MCP hosts' with tags discovery,onboarding.`
8. `Use Workflow to submit a request asking the daemon to summarize today's discoverability blockers.`
