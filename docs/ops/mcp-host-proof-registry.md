...
| OpenAI Apps domain verification | challenge-live; dashboard verify pending | 2026-05-02T14:40-07:00: PR #204 merged; deploy-site run `25262547528` passed; `https://tinyassets.io/.well-known/openai-apps-challenge` returned the OpenAI token | Click dashboard `Verify Domain` only with action-time approval |
...
| ChatGPT App Directory | app draft; submit blocked | `chatgpt-app-submission.json` covers the 11 directory tools with 10 positive and 4 negative tests; 2026-05-02 dashboard draft uses `/mcp-directory`; PR #184 strict redaction deploy/proof and ChatGPT web read/write proof are green; legal/connect closeout landed; OpenAI challenge URL is live; dashboard `Verify Domain` still needs approval; final submit remains blocked on mobile proof, legal/publisher assertions, and action-time host approval |
