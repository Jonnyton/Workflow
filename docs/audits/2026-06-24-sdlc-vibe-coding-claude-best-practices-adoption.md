...
| ID | Item | Owner | Note |
|---|---|---|---|
| R1 | **Coding-loop eval gate** (output + trajectory). Codex opposite-provider review DONE — **ADAPT** (2026-06-24, via `mcp__codex__codex`): 3 adaptations gate the build (S2 wiring is `auto_ship.validate_ship_request`, not a `release_safety_gate`; S1 must stay CI-local; S3 needs its own trajectory schema) | navigator (incorporate the 3 adaptations) | Design note + §Codex review: `docs/design-notes/2026-06-24-coding-loop-eval-gate-wiring.md`. Rubric grounded in the whitepaper's 5 dimensions (task success / tool-use / trajectory / hallucination / response quality). Highest leverage. |
