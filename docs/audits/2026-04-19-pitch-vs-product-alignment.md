...
|---|---|---|---|---|
| C1 | "Zero install. Just paste one URL." | (b) MVP | `mcp.tinyassets.io/mcp` gateway ships at track C MVP. (Pre-outage drafts named `api.tinyassets.io/mcp`; that subdomain was never shipped — actual tunnel hostname is `mcp.`; `api.` reserved for future alias.) | Keep. |
| C2 | MCP URL = `https://mcp.tinyassets.io/mcp` | (c) v1.1 preview | Gateway spec is #27 exec plan; rewrite is post-MVP boundary-crossing work. Legacy `workflow/universe_server.py` surface is what lives at that URL today via tray. | **Gap**: URL is live today but backing implementation is legacy. Doesn't affect users (surface contract matches). No copy change needed; note in launch readiness. |
