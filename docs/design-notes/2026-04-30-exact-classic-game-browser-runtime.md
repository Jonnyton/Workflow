...
4. If required system firmware is proprietary, import it only from a user-owned local file in the browser session.
5. Label any remake, reimplementation, or compatibility port as a fallback, never as the exact-game success state.
...
- Browser runtime: vAmigaWeb can run Amiga disk media in an iframe and supports same-site disk injection plus user-provided Kickstart injection.
- Asset handling: the v1.90 ADF is stored under `/play/scorched-tanks/assets/` with hashes in `SOURCES.md`.
- Firmware handling: no Kickstart ROM is bundled. The page first tries open AROS and then accepts a local user-owned ROM file if the open path is not compatible.
