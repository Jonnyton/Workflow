...
- [Privacy Mode Design-Time Selection](proposed/2026-05-08-privacy-mode-design-time-selection.md) - **status: proposed.** Repository-side response to Issue #442 / WIKI-PATCH. Reframes "privacy mode" as design-time composition of field visibility, data residency, and community compliance templates. No new `privacy_mode` MCP primitive; platform work is limited to concrete enforcement boundaries.
- [Escalation Replay On Substrate Fix](proposed/2026-05-06-escalation-replay-on-substrate-fix.md) - **status: proposed.** Design-only response to wiki patch request PR-053. Adds an operational replay shape for stale terminal handoffs after substrate fixes without a new user-facing MCP action.
...
- [Private Universes: Payload-Redacted Enforcement Under Claude.ai Chat](2026-04-18-privacy-modes-for-sensitive-workflows.md) - **status: historical.** Threat-model background for Claude.ai-mediated private work. Superseded in part by PLAN scoping rules, host-resident private-data design, and the proposed Issue #442 privacy-mode design-time-selection note. Do not dispatch broad `sensitivity_tier` / `private_output` / redactor work from this note alone.
