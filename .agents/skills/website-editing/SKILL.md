...
- **Stage rails stay compact.** A 1-N workflow rail is navigation/status, not the detail pane. Do not let stage tiles stretch to match a tall neighboring event stream or carry full error/output text. Keep tiles compact, clamp long labels, and route verbose failure/output details to the current-run card or event stream. If there is room, include a one-line latest-event summary such as `Last event: ran - intake_router`; never put raw prompt/output JSON in the rail. For historical/no-active-run stages, preserve recency in the tile with `Last ran <timestamp>` and put `See recent events` as the bottom cue instead of replacing the timestamp with a generic `status - see recent events` line.
