"""Auto-prune Docker images when root disk usage crosses a threshold.

BUG-023 Phase 4 — last-line-of-defense against the silent disk-fill
outage class (2026-04-23 P0: 18h dark before host noticed). Runs
``docker image prune -af`` when root-volume usage is at or above
``DISK_AUTOPRUNE_PCT`` (default 85%). Logs the action + outcome.
...
def _docker_image_prune(timeout: float = 120.0) -> tuple[int, str, str]:
    """Run ``docker image prune -af``. Returns (rc, stdout, stderr)."""
    proc = subprocess.run(
...
def check(
    ...
    *,
    ...
    """Prune Docker images when disk usage meets or exceeds threshold.
    ...
    _prune = prune_fn or _docker_image_prune
    ...
    print(
        f"[disk-autoprune] TRIGGER: {path} at {pct:.1f}% — pruning Docker"
    )

    if dry_run:
        print("[disk-autoprune] DRY_RUN: would run `docker image prune -af`")
        return 1
