...
| Community change loop — live proof landed: BUG-044 -> GH #89 -> auto-fix -> `needs-human`; next: writer backend + non-bug request sync. | docs/exec-plans/active/2026-04-30-live-community-reiteration-loop.md, scripts/wiki_bug_sync.py, .github/workflows/{auto-fix-bug,actionlint}.yml | #18 for test edits; writer auth/bridge | dev-ready |
| Community request sync — generalize wiki->GitHub bridge beyond BUG pages, no runtime/test files. | scripts/wiki_bug_sync.py, .github/workflows/wiki-bug-sync.yml, .github/workflows/auto-fix-bug.yml, .agents/.wiki_change_sync_seen.json, docs/exec-plans/active/2026-04-30-live-community-reiteration-loop.md | broad row above; no #18 writes | claimed:codex-gpt5-desktop |
