...
| Community change loop — prod worker green; local tray startup disabled and tray tunnel now opt-in; redeploy latest main, then clear watcher red: stale outage/canary + old queue + Actions PR setting. | Dockerfile, .github/workflows/auto-fix-bug.yml, .github/workflows/deploy-prod.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow_tray.py, workflow/{work_targets.py,api/universe.py}, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/{work_targets.py,api/universe.py}, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{auto_fix_workflow,deploy_prod_workflow,verify_llm_binding,universe_server_telemetry}.py, docs/ops/auto-fix-runbook.md, STATUS.md | - | claimed:codex-gpt5-desktop |
