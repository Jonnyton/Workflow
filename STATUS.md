...
- [2026-04-14] **Phase G.1 follow-ups (5, non-blocking)** — reviewer-flagged at G.1 audit `03e5582`: (1) **G.2 real bug** — `fantasy_author/__main__.py:359-371` fallback (`existing.status == "open" → bid = existing`) lets B re-execute in multi-daemon race when B's local origin ref is stale vs remote. Fix: restrict to `if not _git_has_remote(repo_root)`. Tracked at task #23. (2) Producer-side sandbox layers 1+2 `NodeBidProducer(node_lookup_fn=None)` default means producer accepts adversarial bids; executor catches at run time. Wire in Phase H when host dashboard makes registry observable. (3) `_revert_claim` does `git reset --hard origin/<branch>` — discards uncommitted user changes. Daemon working trees should not share state with manual edits; document in `node_bid_conventions.md` + `bids/README.md`. (4) `_BID_DANGEROUS_PATTERNS as _DANGEROUS_PATTERNS` alias in executors/node_bid.py is a readability trap. Drop alias; use full name. (5) Multi-daemon git race not tested (single-process push-fail simulation only); stress test required before flag-flip — user-sim Mission 9+.
