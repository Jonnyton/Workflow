...
| Daemon claim-time runtime law - branch `codex/daemon-claim-law` / `../wf-daemon-claim-law`; enforce directed daemon, domain-claim, claim-proof, and borrowed-role eligibility in queue/dispatcher without touching daemon wiki. | STATUS.md, .agents/worktrees.md, workflow/branch_tasks.py, workflow/dispatcher.py, fantasy_daemon/__main__.py, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/{branch_tasks.py,dispatcher.py}, tests/test_dispatcher_daemon_identity.py | opposite-family review before landing | claimed:codex-gpt5-desktop |
