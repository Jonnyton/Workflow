...
- [2026-04-14] **Phase E follow-ups (3, non-blocking)** — reviewer flagged at `29a71a7` audit: (1) `queue_cancel` on running tasks returns `running_tasks_require_host_override` rejection with no graph-level interrupt — use daemon pause/stop for in-flight interrupts; real cancel semantics land in Phase H; (2) Missing producer-registry-boundary test per preflight §4.4 invariant 3 — coverage gap, not correctness gap (structural separation clean in code); file as test add-on task; (3) Dispatcher-to-Phase-D invocation plumbing deferred — under D-on+E-on the dispatcher still runs observational-only (not feeding `_run_graph`'s `initial_state`). Wire-up lands when user-sim Mission 8 gates the Phase D default flip, or as Phase F scope (goal-pool needs similar plumbing).
...
| **Phase F** — Goal subscription + pool producer | per rollout plan §Phase F | Phase E landed (`29a71a7`) | pending | First time daemon leaves the universe boundary. May need async TaskProducer protocol variant per spec §1.1. |
