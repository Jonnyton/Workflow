...
| Community change loop — live: queue is draining; watcher dispatches writer on red queue; post-Codex verification gate shipped in 38fdbad; connector opens fallback PRs while Actions PR setting is blocked. | Dockerfile, .github/workflows/{auto-fix-bug,community-loop-watch,deploy-prod,uptime-canary}.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow_tray.py, workflow/{work_targets.py,api/universe.py}, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/{work_targets.py,api/universe.py}, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{auto_fix_workflow,deploy_prod_workflow,verify_llm_binding}.py, tests/test_dns_canary_workflow.py, docs/ops/auto-fix-runbook.md, STATUS.md | - | claimed:codex-gpt5-desktop |
