...
| **Phase G** — NodeBid executor + paid priority weights | `workflow/node_bid.py`, `workflow/executors/node_bid.py`, `workflow/producers/node_bid.py`, `workflow/dispatcher.py`, `workflow/universe_server.py`, `fantasy_author/__main__.py`, `bids/` dir, tests | Phase F landed (`1d02903`) | claimed:claude-code | Reviewer audit: PASS-WITH-NOTES. 50 tests passing. Implementation at `f424646`+`a1d716f`. Flag `WORKFLOW_PAID_MARKET=off` default. No wallet integration — priority slot only. |
