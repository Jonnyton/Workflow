...
- [filed:2026-04-30 verified:2026-05-01] Scorched Tanks not done: must prove live play by hitting a tank with sound; user saw restart after start.
...
| Community change loop — live: queue is draining; watcher dispatches writer on red queue; connector opens fallback PRs while Actions PR setting is blocked. | Dockerfile, .github/workflows/{auto-fix-bug,community-loop-watch,deploy-prod,uptime-canary}.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow_tray.py, workflow/{work_targets.py,api/universe.py}, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/{work_targets.py,api/universe.py}, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{auto_fix_workflow,deploy_prod_workflow,verify_llm_binding,universe_server_telemetry}.py, tests/test_dns_canary_workflow.py, docs/ops/auto-fix-runbook.md, STATUS.md | - | claimed:codex-gpt5-desktop |
| Enable Actions PR creation for auto-fix so `GITHUB_TOKEN` opens PRs without connector fallback. | GitHub repo settings | PR #100/#104-#106 prove branch push works but PR creation blocked | host-action |
