...
- [2026-04-14] **Phase G carry-forward follow-ups (3, non-blocking)** — unchanged after G.2: (1) Producer-side sandbox layers 1+2 `NodeBidProducer(node_lookup_fn=None)` default means producer is no-op; executor catches at run time. Wire in Phase H. (2) `_revert_claim` does `git reset --hard origin/<branch>` — daemon working trees should not share state with manual edits; document in `node_bid_conventions.md` + `bids/README.md`. (3) Multi-daemon git race not tested (single-process push-fail simulation only); stress test required before flag-flip — user-sim Mission 9+. (4) Optional: rename `bid_ledger.py` → `bid_execution_log.py` for disambiguation from settlements ledger (cosmetic, Phase H cleanup).
