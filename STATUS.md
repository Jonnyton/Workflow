...
- [filed:2026-04-30 verified:2026-05-01] Scorched Tanks restart-loop patch local-green; live deploy/reprobe pending; Kickstart blocker remains.
...
| Community change loop — prod worker + uptime canary green; local tray tunnel opt-in; clear remaining watcher red: old queue + Actions PR setting. | Dockerfile, deploy/docker-entrypoint.sh, .github/workflows/{auto-fix-bug,deploy-prod,uptime-canary}.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow_tray.py, workflow/{work_targets.py,api/universe.py}, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/{work_targets.py,api/universe.py}, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{auto_fix_workflow,deploy_prod_workflow,verify_llm_binding,universe_server_telemetry}.py, tests/test_{dns_canary_workflow,dockerfile_shape,env_unreadable_marker}.py, docs/ops/auto-fix-runbook.md, STATUS.md | - | claimed:codex-gpt5-desktop |
| Enable Actions PR creation for auto-fix so `GITHUB_TOKEN` opens PRs without connector fallback. | GitHub repo settings | PR #100 / run 25197746663 prove branch push works but PR creation blocked | host-action |
