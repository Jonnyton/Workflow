...
| Community change loop — live ChatGPT user-sim found missing PR/issue/PLAN review context; shipping `community_change_context` primitive + skill preflight. | Dockerfile, .github/workflows/{auto-fix-bug,community-loop-watch,deploy-prod,uptime-canary}.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow_tray.py, workflow/{work_targets.py,api/universe.py,universe_server.py}, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/{work_targets.py,api/universe.py,universe_server.py}, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{api_universe,mcp_dispatch_docstring_parity,auto_fix_workflow,deploy_prod_workflow,verify_llm_binding,dns_canary_workflow}.py, docs/ops/auto-fix-runbook.md, .agents/skills/ui-test/SKILL.md, .claude/skills/ui-test/SKILL.md, output/user_sim_session.md, STATUS.md | #18 broad lock overlaps docstring/tests; live-loop uptime path proceeds by host direction | claimed:codex-gpt5-desktop |
