...
- [2026-04-14] **Phase 6.2 follow-ups** — 6.2.1 landed at `a657a92` (items 2 branch_rebound guard + 3 gates_disabled fallback). Remaining: (1) private-Branch visibility filter at task #8 blocked on host direction; (4) double-retract idempotency deferred to 6.3 (cosmetic). Reviewer self-corrected mid-audit that handler-level branch_rebound guard did ship at 81affa5 — 6.2.1 adds the storage-layer companion.
- [2026-04-14] **Phase D preflight PASSED reviewer** — `docs/specs/phase_d_preflight.md` revised + committed at `93f108c` after Audit 2 surfaced a contract hole (`node.domain_id` AttributeError — domain_id lives on BranchDefinition, not NodeDefinition) plus pause/stop semantics under flag-on. Fix: Option B threading adds `domain_id: str = ""` kwarg to `_build_node`; `compile_branch` passes `branch.domain_id` at single call site ~graph_compiler.py:559. Pause/stop regression accepted for v1 per §4.10 (wrapper-boundary granularity only). R11 compile-failure hard-fail added. Reviewer re-audit verdict: **PASS**. Dev can claim Phase D after 6.2.1 lands. Phase D Work row path `domains/fantasy_author/__main__.py` is wrong — real path is `fantasy_author/__main__.py` (needs edit next time the row is touched).
