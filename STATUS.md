...
| Community change loop — worker state repair: rehydrate synth signals, clear stale self-stop, prove fresh live activity. | Dockerfile, .github/workflows/auto-fix-bug.yml, .github/workflows/deploy-prod.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow/{work_targets.py,api/universe.py}, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/{work_targets.py,api/universe.py}, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{auto_fix_workflow,deploy_prod_workflow,verify_llm_binding,universe_server_telemetry}.py, docs/ops/auto-fix-runbook.md, STATUS.md | - | claimed:codex-gpt5-desktop |
| Enable Actions PR creation for auto-fix so `GITHUB_TOKEN` opens PRs without connector fallback. | GitHub repo settings | PR #96 / run 25194444920 prove current 403 | host-action |
