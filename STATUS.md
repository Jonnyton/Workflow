...
| **#18 retarget sweep + Arc A/E shim deletion** - lead override; Codex clearing for loop uptime. | workflow/universe_server.py, workflow/api/{evaluation,market,runs,status,helpers}.py, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/, tests/, workflow/storage/__init__.py | - | claimed:codex-loop-uptime ACTIVE 2026-05-02 |
...
| Daemon soul/memory PR #140 - draft; local daemon checks green, CI blocked on plugin mirror until #18 releases runtime. | docs/souls/, workflow/daemon_memory.py, workflow/daemon_registry.py, workflow/daemon_wiki.py, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/ | #18 | dev-ready |
...
| Worker invariant deletion guard - fail pre-commit if staged worker.js has no readable staged content. | STATUS.md, scripts/pre_commit_invariant_worker.py | clear | claimed:codex-gpt5-desktop |
