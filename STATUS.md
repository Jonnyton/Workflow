...
- [2026-04-14] **Phase 6.2 follow-ups (4)** — reviewer PASS-WITH-NOTES on `81affa5`, all non-blocking: (1) `list_gate_claims` + `gates_leaderboard` don't filter private-branch entries (spec §6.2 Integration required filtering; surfaces when private branches exist). (2) `claim_gate` UPDATE silently rewrites `goal_id` on Branch rebind — spec §6.2 6.1 Debt #2 wanted explicit `branch_rebound` rejection; silent rewrite = data-movement spec said to reject; leaderboard for old Goal silently loses claims on rebind. (3) With `GATES_ENABLED=0`, goals leaderboard metric=outcome returns empty entries instead of a flag-gated message. (4) Double-retract returns refreshed retracted row rather than `{status: already_retracted}` — cosmetic. Items 1-2 should land as 6.2.1 patch; 3-4 can fold into 6.3.
- [2026-04-14] **Phase D preflight ready** — `docs/specs/phase_d_preflight.md` landed (not yet committed). Planner flagged one architecturally load-bearing finding: rollout plan's option (b) text ("single source_code node wraps build_universe_graph") is uncompilable under `graph_compiler._validate_source_code` (blocks `__import__`). Revised design introduces "domain-trusted opaque node" as third branch in `compile_branch._build_node`, resolved via `(domain_id, node_id) → Callable` registry in `fantasy_author/branch_registrations.py`. Strictly additive; no field changes to `BranchDefinition`. Awaiting reviewer audit (Audit 2) before dev claims Phase D. Note: STATUS Work row referenced `domains/fantasy_author/__main__.py` but real path is `fantasy_author/__main__.py`.
...
|------|-------|---------|--------|-------|
| **#56 Phase 6.2.1** — patch follow-ups (private-branch visibility + branch_rebound guard) | `workflow/author_server.py`, `workflow/universe_server.py`, tests | 6.2 landed (`81affa5`) | pending | Reviewer-flagged spec deviations from 6.2 audit. (1) `list_gate_claims`/`gates_leaderboard` filter private-branch entries unless caller is owner/host. (2) `claim_gate` UPDATE rejects with explicit `branch_rebound` envelope when claim's denormalized `goal_id` diverges from current branch binding (preferred spec Option 1; current behavior silently rewrites goal_id — data-movement the spec said to reject). Small, isolated patch. |
...
1. **User-sim Mission 8** — re-test sporemarch queue drainage (fix a+b now both landed: `997f825`, `0db181c`) AND verify dispatch_execution request_type routing (fix `68400bc`). Validate reviewer's multi-scene overshoot concern on fix (b) in live.
2. **#56 Phase 6.2** — continues the outcome-gates rollout.
3. The big Phase D-H sequence is the load-bearing user-facing payoff arc — Phase D in particular has high blast radius, plan carefully.
