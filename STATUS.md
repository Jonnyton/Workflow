...
| Community change loop — worker state repair: rehydrate synth signals, clear stale self-stop, prove fresh live activity. | Dockerfile, .github/workflows/auto-fix-bug.yml, .github/workflows/deploy-prod.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow/work_targets.py, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/work_targets.py, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{auto_fix_workflow,deploy_prod_workflow,verify_llm_binding}.py, docs/ops/auto-fix-runbook.md, STATUS.md | - | claimed:codex-gpt5-desktop |
