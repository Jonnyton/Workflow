...
| **Phase E** — tier-aware DaemonController + BranchTask queue | `workflow/branch_tasks.py`, `workflow/dispatcher.py`, possibly `workflow/_file_lock.py`, `workflow/runs.py`, `fantasy_author/__main__.py`, `workflow/universe_server.py`, `tests/test_phase_e_dispatcher.py` | Phase D landed (`c5f29bb`), preflight at `fc565b4` | claimed:dev-2 (session 3) | First time user can see + steer a daemon queue. Major UX surface delivery. Phase E introduces codebase's first file-lock primitive. Two-flag matrix (WORKFLOW_UNIFIED_EXECUTION × WORKFLOW_DISPATCHER_ENABLED) enables staged rollout: D-off+E-on is observational-only. |
