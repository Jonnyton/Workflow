...
- [filed:2026-04-30 verified:2026-05-01] Scorched exact blocked: AROS stops at shell; live compat tank-hit+audio green; no real-user use seen; needs rights-cleared Kickstart.
...
|------|-------|---------|--------|
| Scorched exact-original proof — live compatibility-port proof exists; exact acceptance still needs original Amiga path with input, sound, and tank hit. | WebSite/site/static/play/scorched-tanks/, .agents/skills/classic-game-design-test/, .claude/skills/classic-game-design-test/, STATUS.md | rights-cleared Kickstart-compatible firmware or alternate legal runtime | host-action |
...
| Community change loop + developer soul default — developer daemon live; chatbot-first host control design live @01d9b98; deploy/canary green 2026-05-01Z; ChatGPT user-sim found missing PR/issue/PLAN review context; loop watch red on #39 queue. | PLAN.md, Dockerfile, .dockerignore, .github/workflows/{auto-fix-bug,community-loop-watch,deploy-prod,uptime-canary}.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow_tray.py, workflow/{work_targets.py,api/universe.py,universe_server.py,daemon_registry.py}, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/{work_targets.py,api/universe.py,universe_server.py,daemon_registry.py}, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{api_universe,mcp_dispatch_docstring_parity,dockerfile_shape,auto_fix_workflow,deploy_prod_workflow,verify_llm_binding,dns_canary_workflow,daemon_registry,fantasy_daemon_loop_identity}.py, docs/ops/auto-fix-runbook.md, docs/souls/workflow-developer-daemon.md, .agents/skills/ui-test/SKILL.md, .claude/skills/ui-test/SKILL.md, output/user_sim_session.md, STATUS.md | wait clean loop signal/user evidence; implement ownership-scoped control API; #18 broad lock overlaps docstring/tests by host direction | claimed:codex-gpt5-desktop |
