...
|------|-------|---------|--------|
| Site wiki live-lens refactor — `codex/site-wiki-live-lens` in `../wf-site-wiki-live-lens`; draft PR expected. Keep `/wiki` canonical, `/goals`/`/catalog` aliases only, and retarget visible copy/actions to community wiki + work targets while preserving raw MCP `goals` protocol evidence. Memory refs: `_PURPOSE.md`, `WebSite/DEPLOY.md`, website skill. Related: BUG-034 legacy Goals-router concern + directory/site-cert lanes. | STATUS.md, _PURPOSE.md, WebSite/site/ | none; `.agents/worktrees.md` append deferred behind worktree-discipline lane | claimed:codex-gpt5-desktop-website ACTIVE 2026-05-02 |
...
| Community loop canonical invoke smoke - parent `4da58d59ff4c4e18` spawned child `c788bb7380484196`; child stuck at `keep_reject_gate` after Codex/bwrap failure; fix provider fail-loud/runtime then rerun. | workflow/providers/{base,codex_provider}.py, workflow/runs.py, workflow/graph_compiler.py, workflow/api/runs.py, tests/test_{providers,sandbox_unavailable,node_timeout,sub_branch_invocation,wait_for_run}.py, docs/exec-plans/active/2026-04-30-live-community-reiteration-loop.md, MCP live branches fd5c66b1d87d/e019229850f9 | API/tests claim; Claude review when back | claimed:codex-loop-uptime-chatgpt |
