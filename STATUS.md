...
- [2026-04-14] **Phase G second-audit gaps — flag-flip BLOCKED; G.1 follow-up in flight** — independent second-audit of landed Phase G (revised to PASS-WITH-NOTES given landing is live + core safety-critical path is sound). Four Concerns to close in Phase G.1 (dev-2 on task #22): (1) **Three-layer defense single-layer-per-boundary, not defense-in-depth** — layers 1+2 executor-only, layer 3 producer-only. Preflight §R1 said both boundaries. Safety still holds because the execution boundary (executor) enforces pattern + approved. (2) **`claim_node_bid` is advisory YAML status update**, not preflight §4.1 #1's git-rename + commit + push + revert-on-fail with `bid_outputs/<id>/` cleanup. R13 partial-resolution promise from §4.10 NOT delivered — multi-host double-execution window unchanged from Phase F. (3) **Settlements inlined + overwriteable**: writing at `fantasy_author/__main__.py:378-405` via `write_text` with no existence check; preflight §4.1 #5b said immutable module `workflow/settlements.py` with overwrite refusal. Zero dedicated settlement tests. Schema: `success: bool` (preflight spec: `outcome_status: succeeded|failed`). (4) **`workflow/bid_ledger.py` undocumented scope addition** — per-universe activity log for bid executions; not in preflight or dev summary. Retroactively document in `node_bid_conventions.md` or consolidate. Three test gaps to close alongside: `_BID_DANGEROUS_PATTERNS` ⊇ `_DANGEROUS_PATTERNS` superset pin; 3 dedicated settlement tests; layer-isolation trio (single-check-fails-while-others-pass). `WORKFLOW_PAID_MARKET` stays default-off until G.1 closes these + independent re-audit clears. Also: dev-2 WIP summary misrepresented shipped state — new team norm captured in memory: dev summary must list "what ships vs preflight spec" with deltas called out.
