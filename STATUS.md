...
| **Reviewer concern follow-ups (trivial cluster)** | `workflow/graph_compiler.py` comment, `workflow/storage/backend.py` typing, `workflow/runs.py` docstring | - | pending | Reviewer verified against HEAD: (2) add caveat comment to `_TIMEOUT_EXECUTOR` about queued-call timeout measurement. (3) `stage_hook: Any` → `Callable[[Path], None] \| None` in `SqliteCached`. (5) doc sentence in `stream_run`/`await_run_events` pinning step_index as opaque cursor, not ordinal. Concerns #1 and #4 already resolved in Phase 7.1. |
...
| **Orphaned test_phase7.py cleanup** | `tests/test_phase7.py` | - | pending | 4 tests in `TestDaemonControllerIntegration` call methods that never existed on main (`_bootstrap_universe_runtime_files`, `_bootstrap_retrieval_indices`, `_run_progressive_ingestion`). Planner decision memo recommends delete (not re-scope). ~110 lines to remove. Keep the other TestProgressiveIngestor/TestOutputVersionStore/TestSeriesPromiseTracker classes — real modules. |
...
| **#56** Phase 6+: Outcome gates | new schema, `workflow/universe_server.py` new `gates` tool | #54 shipped | pending (spec-first) | Per PLAN.md "Outcome gates". Self-report first, automation later. Planner confirmed #56 fully untouched in code. |
