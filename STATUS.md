...
| Community change loop — live user proof + branch-skill primitive: users can create/remix/copy skills and attach skill snapshots to branches. | PLAN.md, Dockerfile, .dockerignore, .github/workflows/{auto-fix-bug,community-loop-watch,deploy-prod,uptime-canary}.yml, scripts/{community_loop_watch,verify_llm_binding}.py, workflow_tray.py, workflow/{branches.py,daemon_server.py,work_targets.py,api/{branches,universe,prompts}.py,universe_server.py,daemon_registry.py,catalog/serializer.py}, packaging/claude-plugin/plugins/workflow-universe-server/runtime/workflow/, fantasy_daemon/{__main__,branch_registrations}.py, tests/test_{branch_skills,api_universe,mcp_dispatch_docstring_parity,dockerfile_shape,goals_discoverability,universe_server_metadata,auto_fix_workflow,deploy_prod_workflow,verify_llm_binding,dns_canary_workflow,daemon_registry,fantasy_daemon_loop_identity}.py, docs/ops/auto-fix-runbook.md, docs/souls/workflow-developer-daemon.md, .agents/skills/ui-test/SKILL.md, .claude/skills/ui-test/SKILL.md, output/user_sim_session.md, STATUS.md | wait clean loop signal/user evidence; ownership-scoped control API next; #18 broad lock overlaps docstring/tests by host direction | claimed:codex-gpt5-desktop |
