...
- [2026-04-14] **Worktree `claude/inspiring-newton` retire** — DONE (2026-04-14, elastic-jennings session). Force-removed via `git worktree remove --force` + `git worktree prune`. Agent defs in worktree were stale vs. main (main was strictly ahead). Concern resolved.
...
- [2026-04-14] **Phase D follow-ups (3 remain, non-blocking)** — reviewer-flagged at `c5f29bb` audit: (1) User-registered Branches with body-less nodes (if any exist in prod SQLite registry) will fail compile on load — low-probability but release notes should mention; (2) Boundary-field count inconsistency across preflight §4.3 invariant 1 (6 fields) / seed YAML output_keys (7) / `_BOUNDARY_FIELDS` in wrapper (10) — doc-level only, implementation is correct per 6-field restore test; (3) Missing-yaml vs registry-miss hard-fail paths share contract but raise different exception types (FileNotFoundError vs CompilerError) — cosmetic. Item 4 (activity.log byte-parity automation) → **RESOLVED**: `tests/test_phase_h_activity_log_parity.py` is the Phase H target for this (R6, invariant 7 per preflight); Phase D follow-up #4 is closed when that test file lands. Items 1-3 fold into docs-pass before flag-default flip.
...
- [2026-04-14] **Phase G R5: NodeBid double-execution — RESOLVED for remote-configured repos** — G.2 (`20e8886`) closed the race-bypass bug (fallback path short-circuits to `claim_race_lost` on remote repos). Phase H stress test (`tests/test_phase_h_claim_stress.py`) provides 6-scenario coverage including stale-origin-ref regression pin. Multi-process race not yet stress-tested end-to-end; gates `WORKFLOW_PAID_MARKET` default flip (Mission 10).
...
- [2026-04-14] **Phase G.1 landed at `03e5582`** — closes 4/4 Phase G blockers from second audit (git-rename claim atomicity with NodeBid | None return + _revert_claim cleanup; workflow/settlements.py module with SettlementExistsError immutability; producer-side sandbox layers 1+2 with node_lookup_fn hook; _BID_DANGEROUS_PATTERNS single-source consolidation). 14 new tests; reviewer PASS-WITH-NOTES. `WORKFLOW_PAID_MARKET` still default-off pending G.2 + user-sim Mission gate.
- [2026-04-14] **Phase G.2 landed at `20e8886`** — closes the race-bypass bug (fallback path now short-circuits to `claim_race_lost` on remote-configured repos; local-test ergonomics preserved when no remote). Cosmetic alias cleanup (`_BID_DANGEROUS_PATTERNS` no longer aliased in executor). Reviewer PASS. R5 "partial-resolution" preflight promise now DELIVERED for production remote-configured repos.
- [2026-04-14] **Phase G carry-forward follow-up (1 remains, non-blocking)** — G.2 items: (1) node_lookup_fn wiring → **RESOLVED** at `2572154` (Phase H wired `NodeBidProducer(node_lookup_fn=...)` at daemon startup; producer sandbox layers 1+2 now load-bearing). (3) Multi-daemon git race stress test → **RESOLVED** via `tests/test_phase_h_claim_stress.py` (Phase H, 6 `@pytest.mark.slow` scenarios per preflight R5). (4) bid_ledger rename → **RESOLVED** at `2572154` (bid_ledger.py → bid_execution_log.py, deprecation shim retained). Remaining: (2) `_revert_claim` does `git reset --hard origin/<branch>` — daemon working trees should not share state with manual edits; document in `node_bid_conventions.md` + `bids/README.md` before `WORKFLOW_PAID_MARKET` default flip.
