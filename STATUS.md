...
- [filed:2026-07-01 verified:2026-07-01] `claude/founder-identity-allslices`: private-universe run leaks via `query_runs`/`get_routing_evidence` FIXED (query_runs `row_filter` before projection/aggregation; routing-evidence record filter). All wiki + run read/write surfaces now per-universe ACL-gated. Residual = cross-surface ACL audit of OTHER tools (gates/market/escrow/messaging/project_memory/canon) — scope as one comprehensive pass, and live WorkOS rollout (canary + ui-test).
