...
from fantasy_author.nodes.world_state_db import connect, get_all_facts, init_db
    ...
    if generated_files:
        ...
        if universe_path:
            try:
                ...
                if new:
                    ingestor.triage()
                    from fantasy_author.memory.ingestion import IngestionPriority
...
def _handle_new_element(
    ...
    from fantasy_author.nodes._provider_stub import call_provider, last_provider
...
def _handle_synthesize_source(
    ...
    if generated:
        from fantasy_author.ingestion.core import SourceManifest
...
def _run_leiden_clustering(state: dict[str, Any]) -> None:
    ...
    try:
        from fantasy_author.knowledge.leiden import detect_communities_from_kg
