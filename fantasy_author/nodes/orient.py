...
from fantasy_author.nodes._paths import resolve_db_path as _resolve_db_path
...
# Maximum bounded reflection passes when gaps are detected in retrieval.
# Prevents unbounded re-query loops.
_MAX_REFLECTION_PASSES = 2
...
def _detect_premise_mismatch(
    ...
    """
    from fantasy_author.evaluation.structural import _extract_premise_terms
...
def orient(state: dict[str, Any]) -> dict[str, Any]:
    ...
    # Bounded reflection: re-query up to _MAX_REFLECTION_PASSES times if
    # critical gaps are detected in the initial retrieval.
    all_gaps: list[RetrievalGap] = []
    reflection_passes = 0
    while reflection_passes < _MAX_REFLECTION_PASSES:
...
def _extract_pov_and_tier(
    ...
    try:
        from fantasy_author.nodes.world_state_db import get_all_characters
...
def _run_retrieval(
    ...
    from fantasy_author.retrieval.agentic_search import run_phase_retrieval
...
def _build_orient_query(state: dict[str, Any], scene_id: str) -> str:
    ...
    from fantasy_author.retrieval.agentic_search import build_phase_query
